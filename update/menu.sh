...
UPDATE="https://raw.githubusercontent.com/Hyung098/Closed/main/update/update.sh"
GREEN='\033[0;32m'
RED='\033[0;31m'
ipsaya=$(wget -qO- ipinfo.io/ip)
data_server=$(curl -v --insecure --silent https://google.com/ 2>&1 | grep Date | sed -e 's/< Date: //')
date_list=$(date +"%Y-%m-%d" -d "$data_server")
data_ip="https://raw.githubusercontent.com/Hyung098/Zapanes/main/Bill"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/Lawonteam"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/6287845717702"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
...
Name=$(curl -sS https://raw.githubusercontent.com/Hyung098/Zapanes/main/Bill | grep $MYIP | awk '{print $2}')
if [ "$res" = "Expired" ]; then
Exp="\e[36mExpired\033[0m"
else
Exp=$(curl -sS https://raw.githubusercontent.com/Hyung098/Zapanes/main/Bill | grep $MYIP | awk '{print $3}')
fi
...
data_ip="https://github.com/versi01/versi01.github.io/raw/main/ip"
...
echo -e "  ${BLUE}• ${GREEN}Developer           ${NC}= Lawon VPN ${NC}"
echo -e "$COLOR1┌────────────────────────────────────────────────────────────┐${NC}"
...
echo -e "  ${CYAN}[07]${NC} • ${RED}[${NC}${GREEN} INSTALL UDP ${NC}${RED}]${NC}          ${CYAN}[14]${NC} • ${RED}[${NC}${GREEN} EDIT DOMAIN SLOW{NC}${RED}]${NC} "
echo -e "$COLOR1┌────────────────────────────────────────────────────────────┐${NC}"
...
05 | 5) clear ; menu-ss ;;
06 | 6) clear ; wget https://raw.githubusercontent.com/Hyung098/Closed/main/update/update.sh && chmod +x update.sh && ./update.sh ;;
